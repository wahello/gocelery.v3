...
func NewAMQPCeleryBroker(qname string, host string) *AMQPCeleryBroker {
	conn, ch := NewAMQPConnection(host)
	return NewAMQPCeleryBrokerByConnAndChannel(qname, conn, ch)
}

// NewAMQPCeleryBrokerByConnAndChannel creates new AMQPCeleryBroker using AMQP conn and channel
func NewAMQPCeleryBrokerByConnAndChannel(qname string, conn *amqp.Connection, channel *amqp.Channel) *AMQPCeleryBroker {
	broker := &AMQPCeleryBroker{
		Channel:    channel,
		Connection: conn,
		Exchange:   NewAMQPExchange("default"),
		Queue:      NewAMQPQueue(qname),
		Rate:       4,
	}
...
func (c *AMQPCeleryBroker) GetQueueName() string {
	return c.Queue.Name
}
